...
function shape_function_bspline(knot_vectors, orders, control_points, nij, qp)
	...
	#Map the gausspoint from parent domain to parametric domain
	#This is simple since all edges are straight
	ξ = 0.5*((local_knot[ni+1] - local_knot[ni])*gp[1] + (local_knot[ni+1] + local_knot[ni]))
	dξdξ̃ = 0.5*(local_knot[ni+1] - local_knot[ni])

	η = 0.5*((local_knot[nj+1] - local_knot[nj])*gp[2] + (local_knot[nj+1] + local_knot[nj]))
	dηdη̂ = 0.5*(local_knot[nj+1] - local_knot[nj])
	
	#Get basefunction values and its derivative w.r.t parametric coord \xi	
	for (i, global_basefunc) in enumerate(element_basefunctions)
		N[i] = BSplines._bspline_basis_value(p, local_knot, global_basefunc, ξ)
		M[i] = BSplines._bspline_basis_value(p, local_knot, global_basefunc, ξ)
		dNdξ[i] = gradient( (ξ)->BSplines._bspline_basis_value(p, knot_vectors[1], ni, ξ), ξ)
		dMdη[i] = gradient( (η)->BSplines._bspline_basis_value(q, knot_vectors[2], nj, η), η)
	end

	dxdξ = 0.0
	for i in 1:nen
		dxdξ += dNdξ[i]*control_points[i][1]
	end
...
function solve_2d()
	...
	for ie in 1:size(mesh.IEN,2)

		IGA.set_current_element!(ip,ie)

		globalnodes = (mesh.IEN[:,ie])
		nij= mesh.INN[globalnodes[1],:]

		#for d in 1:dim
		#	if mesh.knot_vectors[d][nij[d]+1] == mesh.knot_vectors[d][nij[d]] 
		#		continue
		#	end
		#end
		
		cellvalues = CellVectorValues(qr,ip)
		reinit!(cellvalues, mesh.control_points[globalnodes])


		fill!(Ke, 0.0)
		for iqp in 1:getnquadpoints(cellvalues)
			for i in 1:n_basefunctions
            	ɛ[i] = symmetric(shape_gradient(cellvalues, iqp, i)) 
	        end
